//! 最小 CancellationToken（§15.3 structured cancellation）。
//!
//! §15.3：结构化取消——shutdown 信号必须沿调用栈显式传播，不得依赖
//! 全局状态或隐式超时。本模块提供第一方 ~40 行实现（`tokio::sync::watch`
//! + `AtomicBool`），**不引入 tokio-util**（§23.1：标准库/tokio 现有能力足够时不得新增依赖）。
//!
//! 语义：
//! - [`CancellationToken::cancel`]：至多生效一次（幂等；重复 cancel 无害）；
//! - [`CancellationToken::cancelled`]：等待取消。**无丢失唤醒竞态**：
//!   `watch` 是"值 + 版本"通道——先创建 `subscribe`（读到当前值），
//!   再 `changed()`；cancel 若在 `changed` 挂起后发生，`send` 立即可见；
...
mod tests {
    ...
    #[tokio::test]
    async fn cancel_wakes_all_waiters() {
        let token = CancellationToken::new();
        ...
        for waiter in waiters {
            let result = tokio::time::timeout(Duration::from_secs(5), waiter)
                .await
                .ok();
            assert!(result.is_some(), "all waiters must be woken");
        }
    }
